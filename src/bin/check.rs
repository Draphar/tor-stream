...
fn main() {
    let address = if let Some(address) = args()
        .nth(1)
        .or_else(|| var("TOR_PROXY").ok()) {
...
fn check_clear_web(address: SocketAddr) {
    let mut stream = TorStream::connect_with_address(address, "www.example.com:80").unwrap_or_else(|e| {
    ...
    if buf.starts_with("HTTP/1.1 200 OK") {
        ...
        eprintln!("Clear web check failed\nInvalid response; dump ({} bytes):\n--------\n{}\n--------", buf.len(), buf);
    }
}

fn check_hidden_service(address: SocketAddr) {
    let mut stream = TorStream::connect_with_address(address, ("wlupld3ptjvsgwqw.onion", 80)).unwrap_or_else(|e| {
        if e.kind() == ErrorKind::ConnectionRefused {
            eprintln!("Connection refused, is Tor running?");
        } else {
            eprintln!("Failed to connect: {}", e);
